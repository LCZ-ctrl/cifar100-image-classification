...
class DenseNet169(nn.Module):
    def __init__(self, num_classes=100, growth_rate=32, dropout=0.3):
        ...
        self.block3 = DenseBlock(256, num_layers=32, growth_rate=growth_rate)  # [B, 1280, 8, 8]
        self.trans3 = Transition(in_channels=256 + 32 * growth_rate, out_channels=640)  # [B, 640, 4, 4]

        self.block4 = DenseBlock(640, num_layers=32, growth_rate=growth_rate)  # [B, 1664, 4, 4]

        self.classifier = nn.Sequential(
            nn.BatchNorm2d(640 + 32 * growth_rate),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((1, 1)),  # [B, 1664, 1, 1]
            nn.Flatten(),  # [B, 1664]
            nn.Dropout(dropout),
            nn.Linear(640 + 32 * growth_rate, num_classes)  # [B, 100]
        )
