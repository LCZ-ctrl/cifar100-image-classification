...
class WideResNet_28_10(nn.Module):
    def __init__(self, depth=28, width_factor=10, num_classes=100, dropout=0.3):
        super().__init__()
        ...
        self.layer1 = self._make_layer(channels[0], channels[1], n, stride=1, dropout=dropout)  # [B, 160, 32, 32]
        self.layer2 = self._make_layer(channels[1], channels[2], n, stride=2, dropout=dropout)  # [B, 320, 16, 16]
        self.layer3 = self._make_layer(channels[2], channels[3], n, stride=2, dropout=dropout)  # [B, 640, 8, 8]

        self.bn = nn.BatchNorm2d(channels[3])
        self.relu = nn.ReLU(inplace=True)

        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),  # [B, 640, 1, 1]
            nn.Flatten(),  # [B, 640]
