...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, cardinality=32, width_per_group=4):
        super().__init__()

        hidden_channels = out_channels * (cardinality * width_per_group // 64)
...
class ResNeXt50(nn.Module):
    def __init__(self, num_classes=100, dropout=0.3, cardinality=32, width_per_group=4):
        super().__init__()
        ...
        self.layer3 = self._make_layer(512, 256, num_blocks=6, stride=2)  # [B, 1024, 8, 8]
    ...
    def _make_layer(self, in_channels, out_channels, num_blocks, stride):
        layers = [Bottleneck(in_channels, out_channels, stride=stride, cardinality=self.cardinality,
                             width_per_group=self.width_per_group)]
        new_in_channels = out_channels * Bottleneck.expansion
        for _ in range(1, num_blocks):
            layers.append(Bottleneck(new_in_channels, out_channels, stride=1, cardinality=self.cardinality,
                                     width_per_group=self.width_per_group))
